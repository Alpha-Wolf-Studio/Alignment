...
{
	 void * Alloc( size_t in_uSize )
	{
		return AkAlloc(T_MEMID, in_uSize);
	}

	 void * ReAlloc( void * in_pCurrent, size_t in_uOldSize, size_t in_uNewSize )
	{
		return AkRealloc(T_MEMID, in_pCurrent, in_uNewSize);
	}

	 void Free( void * in_pAddress )
	{
		AkFree(T_MEMID, in_pAddress);
	}

	 void TransferMem(void *& io_pDest, AkArrayAllocatorNoAlign<T_MEMID> in_srcAlloc, void * in_pSrc )
	{
		io_pDest = in_pSrc;
	}
};
...
struct AkArrayAllocatorAlignedSimd
{
	 ...
	 void Free( void * in_pAddress )
	{
	 ...
	 void TransferMem(void *& io_pDest, AkArrayAllocatorAlignedSimd<T_MEMID> in_srcAlloc, void * in_pSrc )
	{
		io_pDest = in_pSrc;
	}
...
struct AkHybridAllocator
{
	 ...
	 void Free(void * in_pAddress)
	{
	 ...
	 void TransferMem(void *& io_pDest, AkHybridAllocator<uBufferSizeBytes, uAlignmentSize, T_MEMID>& in_srcAlloc, void * in_pSrc)
	{
...
struct AkAssignmentMovePolicy
{
	...
	static  void Move( T& in_Dest, T& in_Src )
	{
		in_Dest = in_Src;
	}
...
// Bypass this inner struct and use a proxy in a separate header.
	...
	struct AkIterator
	{
		...
		/// + operator
		AkIterator operator+(AkUInt32 inc) const
		{
			AKASSERT( pItem );
			AkIterator returnedIt;
			returnedIt.pItem = pItem + inc;
			return returnedIt;
		}

        /// - operator
        AkUInt32 operator-(AkIterator const& rhs) const
        {
	...
	AkIterator FindEx( ARG_T in_Item ) const
	{
		AkIterator it = Begin();

		for ( AkIterator itEnd = End(); it != itEnd; ++it )
		{
			if ( *it == in_Item )
				break;
		}
	...
	AkIterator BinarySearch( ARG_T in_Item ) const
	{
	...
	{
		AkIterator AkIterator;
		AkIterator.pItem = m_pItems + in_uIndex;
		EraseSwap(AkIterator);
	}
...
#pragma warning( pop )
	...
	void RemoveAll()
	{
		for ( AkIterator it = Begin(), itEnd = End(); it != itEnd; ++it )
			(*it).~T();
		m_uLength = 0;
	}
